...
- `docker-compose-monitor.yml` - мониторинг
  - `prometheus` - метрики + алертинг
...
- `./schemas/products.json` - для публикации продуктов в топик `products`
- `./schemas/client-search.json` - для публикации пользовательских запросов в топик `client-search`
