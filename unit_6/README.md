...
- Версия решения для работы в облаке Yandex
  ...
  - И в группе практикума куратор сказал, что с NiFi задачу можно сдать в локальном варианте.
...
Сейчас этого делать не будем, просто скопируем нашу настроенную цепочку:
```bash
mkdir -p nifi \
&&  docker cp nifi:/opt/nifi/nifi-current/conf/flow.json.gz ./nifi/flow.json.gz
```

#### Если хотим собрать все окружение с таким процессорами

- пересобрать:
  ```bash
  make rebuild-with-nifi-proc
  ```
- Откройте страницу https://localhost:8443/nifi/
- Запустите цепочку процессоров
- Проверьте результат как было описано выше
